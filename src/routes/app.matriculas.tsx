...
import { mapApiData, pickStr, isValidVin, normalizeFecha } from "@/lib/mptc/plate-map";
export { mapApiData };
export type { MappedPlate } from "@/lib/mptc/plate-map";
...
function MatriculasPage() {
  ...
  // La búsqueda solo se dispara al pulsar el botón "Buscar" (o Enter).
  // Sin debounce automático para evitar consultas con matrículas incompletas.
        ...
        <section className="rounded-2xl border border-border bg-surface p-4 shadow-sm">
          ...
          <ul className="space-y-2">
            {recent.map((r) => (
              <li
                key={r.plate}
                className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-lg bg-surface-2 p-2"
              >
                <button
                  type="button"
